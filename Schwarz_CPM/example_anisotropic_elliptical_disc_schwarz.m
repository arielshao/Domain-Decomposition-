%% Solving an anisotropic elliptic equation on an elliptical disc using Schwarz iterations 
...
%  where \Omega is the elliptical-shape disc consisting of two half
%  elliptical-shape discs
...
cpf{1}=@(x,y) cpEllipticalDisc_left(x,y,a,b,  [0 0], 0.1);
cpf{2}=@(x,y) cpEllipticalDisc_right(x,y,a, b, [0 0], -0.1);
...
%% Find the ghost points for schwarz bc
  for i=1:N
    ...
    % interior of o.
    band=1:1:length(x);
    [C, Io, Is] = intersect(band(~ o.bdy), band(s.bdy));  % find the ghost points
    
    temp= find(~o.bdy);  Io = temp(Io);
    temp = find(s.bdy);  Is = temp(Is);
    assert (all (C == band(Io)));
    assert (all (C == band(Is)));
...
%      E_bdy_in=interp2_matrix(x1d, y1d, s.cpbarx(Is), s.cpbary(Is),  1, s.band);
      E_bdy_in=interp2_matrix(x1d, y1d, s.cpbarx(Is), s.cpbary(Is),  1);
...
%      E_ghost_in= interp2_matrix(x1d, y1d,  s.cpx(Is), s.cpy(Is), 1, o.band);
     E_ghost_in= interp2_matrix(x1d, y1d, s.cpx(Is), s.cpy(Is), 1);
     d{i}.Is=Is;
...
% ------------------------------------------------------------------------------------------
% 0.04        7.28e-3            7.78e-3          9
% 0.02        3.42e-3            3.53e-3          12
% 0.01        1.695e-3           1.72e-3          10
% 0.005       8.391e-4           5.881e-4         11  more accurate 
