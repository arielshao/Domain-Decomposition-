function [cpx,cpy,cpz, dist, bdy] = cpTorus_left(x,y,z, R,r, cen,x_right_end)
% cpTorus_left Closest point function for a torus with right sector cut from
% x_right_end onwards (x_right_end >= cen(1))


%   [cpx,cpy,cpz, dist, bdy] = cpTorus_left(x,y,z, R,r, cen, x_right_end) returns the
%   closest point and distance of (x,y,z) to the Torus centred at cen
%   with major radius R and minor radius r. The right sector is cut from x=x_right_end
%   onwards
...
% from cutting point x=x_right_end 
  ...
  cpy(I)=sign(cpy(I)-eps).*sqrt((R+r*cos(phi)).^2-x_right_end^2);
