% This code aims to implement the Additive Schwarz Method to 
...
% u_Gamma_1=zeros(size(X));
% u_Gamma_2=zeros(size(X));
% u_Gamma_3=zeros(size(X));
% u_Gamma_4=zeros(size(X));
% u_Gamma_5=zeros(size(X));
...
% 
tol=dx/2;
% % Grid points on ring 1 3 and 5
index_1=(sqrt((cpbar_x1-X).^2+(cpbar_y1-Y).^2)-...
       sqrt((X-cp_x1).^2+(Y-cp_y1).^2)<tol);
xx1=X(index_1); yy1=Y(index_1);
 % Interpolate from the embedded space to the grid points on the first ring 
...
% Grid points on ring 2 & 4 but not intersecting ring 1 ,3 and 5
...
% Interpolate from the embedded space to the grid points on the 2nd ring (not interseting ring 1 & 3)
...
xxx4=X(index_44); yyy4=Y(index_44);
 % Interpolate from the embedded space to the grid points on the 4nd disc (not interseting disc 2 & 5)
 Eplot_4=interp2_matrix(x1d,y1d,xxx4,yyy4, p);
disp ('done');

%% Assign true solution 
 
% u_true= @(x,y) ((x+3).^2+y.^2-1).*((x+1.5).^2+(y+0.5).^2-1).*(x.^2+y.^2-1).*...
...
h=colorbar;
title("Plot of the initial guess u0")
xlim([-5, 5])
ylim([-2, 2])
 %% Iterations
 counter=0;
...
for step =1:Nsteps
 ...
 % Solve the pde in each domain using backslash
 u1=M_1\f_1;
 u2=M_2\f_2;
 u3=M_3\f_3;
 u4=M_4\f_4;
...
colorbar;
title("Plot of u^{50} for CP Additive Schwarz Method")
